...
def solicitar_exames(request):
    ...
    if request.method == 'GET':        
        ...
        # TODO: Calcular preco dos exames disponíveis
        preco_total = 0
        for i in solicitacao_exames:
            if i.disponivel:
                preco_total += i.preco
        
        return render(request, 'solicitar_exames.html', {'tipos_exames': tipos_exames, 'solicitacao_exames': solicitacao_exames, 'preco_total': preco_total, 'data_atual': data_atual})
...
def solicitar_senha_exame(request, exame_id):
    ...
    if request.method == 'GET':
        ...
        if os.path.exists(exame.resultado.path):
            ...
            messages.add_message(request, constants.ERROR, 'Arquivo não encontrado!')
